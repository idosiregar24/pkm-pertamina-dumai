...
export default function CategoryEdit({ category }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('admin.kategori.update', category.id));
    };

    if (!category) {
        return (
            <AdminLayout title="Edit Kategori" activeNav="kategori">
                <div className="flex items-center justify-center py-24">
                    <p className="text-sm text-slate-400">Data kategori tidak ditemukan.</p>
                </div>
            </AdminLayout>
        );
    }
    ...
    return (
        <AdminLayout title="Edit Kategori" activeNav="kategori">
            ...
            <div className="flex items-center gap-3 mb-6">
                ...
                <h1 className="text-xl font-extrabold text-slate-900 tracking-tight">Edit Kategori Produk</h1>
            </div>
                    ...
                    <div className="space-y-4">
                        <div className="bg-blue-50 border border-blue-100 rounded-2xl p-4">
                            <div className="flex items-center gap-1.5 mb-2">
                                ...
                                <p className="text-xs font-bold text-[#005BAC]">
                                    {category.products_count ?? 0} produk memakai kategori ini
                                </p>
                            </div>
                            <p className="text-[11px] text-slate-500 leading-relaxed">
                                Mengganti nama otomatis memperbarui label kategori di semua produk terkait, termasuk filter di halaman Katalog publik.
                            </p>
                        </div>
                    </div>
                </div>
